...
*Setup instructions for a gitian build of VIRIDI using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the VIRIDI
...
Clone the git repositories for xdna and gitian and then checkout the xdna version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the xdna repository
...
To build VIRIDI (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the xdna repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/xdna/.git/
...
For example:
```bash
URL=https://github.com/crowning-/xdna.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit xdna=${COMMIT} --url xdna=${URL} ../xdna/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit xdna=${COMMIT} --url xdna=${URL} ../xdna/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit xdna=${COMMIT} --url xdna=${URL} ../xdna/contrib/gitian-descriptors/gitian-osx.yml
```
